<template>
  <el-drawer v-model="drawerVisible" :destroy-on-close="true" size="450px" :title="`${drawerProps.title}用户`">
    <el-form ref="ruleFormRef" label-width="100px" label-suffix=" :" :rules="rules" :model="drawerProps.rowData">
      ...
      <el-form-item label="用户昵称" prop="name" v-if="drawerProps.title !== '分配角色'">
        <el-input v-model="drawerProps.rowData!.name" placeholder="请填写用户昵称" clearable></el-input>
      </el-form-item>
      <el-form-item label="用户密码" prop="password" v-if="drawerProps.title === '新增'">
        <el-input v-model="drawerProps.rowData!.password" placeholder="请填写用户密码" clearable></el-input>
      </el-form-item>
      <el-form-item label="角色列表" prop="rolesId" v-if="drawerProps.title === '分配角色'">
        <el-checkbox v-model="state.checkAll" :indeterminate="state.isIndeterminate" @change="handleCheckAllChange">
          全选
        </el-checkbox>
        <el-checkbox-group v-model="state.assignRoles" @change="handleCheckedChange">
          <el-checkbox v-for="item in state.allRolesList" :key="item.id" :label="item.id">
            {{ item.roleName }}
          </el-checkbox>
        </el-checkbox-group>
      </el-form-item>
...
const handleCheckAllChange = (val: CheckboxValueType) => {
  state.assignRoles = val ? state.allRolesList.map((item: Roles) => item.id) : [];
...
const handleCheckedChange = (value: CheckboxValueType[]) => {
  ...
  state.isIndeterminate = checkedCount > 0 && checkedCount < state.allRolesList.length;
};
