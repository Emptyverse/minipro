<template>
  <div>
    <ProTable
      ...
      :columns="columns"
      :request-api="getAclUserList"
      :init-param="initParam"
    >
      <template #tableHeader="scope">
        <el-button type="primary" icon="Plus" v-auth="['btn.User.add']" @click="openDrawer('新增')"> 添加 </el-button>
        <el-button
          type="danger"
          icon="Delete"
          plain
      ...
      <template #operation="scope">
        <el-button type="primary" link icon="UserFilled" v-auth="['btn.User.assgin']" @click="openDrawer('分配角色', scope.row)">
          分配角色
        </el-button>
...
// *新增、编辑、分配角色
const drawerRef = ref();
const openDrawer = async (title: string, rowData: Partial<AclUser.ResAclUserList> = {}) => {
  let params = {
    title,
    rowData: { ...rowData },
    list: title === "分配角色" ? await getUserRolesList(rowData!.id || "") : [],
    api: title === "新增" ? addAclUser : title === "编辑" ? updateAclUser : assignUserRoles,
